...
export function AccountCard({ account, pending, quotaPending, onEdit, onToggle, onDelete, onRefresh, onRefreshQuota, onSync, onExport, onRelogin }: { account: AuthAccount; pending: boolean; quotaPending: boolean; onEdit: () => void; onToggle: () => void; onDelete: () => void; onRefresh: () => void; onRefreshQuota: () => void; onSync: () => void; onExport: () => void; onRelogin: () => void }) {
  ...
  const supportsQuota = isCodex;
  const supportsExport = isCodex;
  ...
  return <article className={cardClass} aria-label={`${account.label} 认证账号`}>
    ...
    <div className={`mt-4 grid ${!supportsExport && !supportsQuota ? "grid-cols-2" : isCodex ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-3"} gap-2 border-t border-border pt-4`}>{invalid ? <button onClick={onRelogin} disabled={pending} className="col-span-full action-primary justify-center"><KeyRound size={15} />重新登录</button> : null}{supportsQuota && <button onClick={onRefreshQuota} disabled={pending} className="action-secondary justify-center px-1.5 py-2 text-xs">{quotaPending ? <Loader2 size={14} className="animate-spin shrink-0" /> : <Gauge size={14} className="shrink-0" />}刷新额度</button>}<button onClick={onRefresh} disabled={pending} className="action-secondary justify-center px-1.5 py-2 text-xs">{pending && !quotaPending ? <Loader2 size={14} className="animate-spin shrink-0" /> : <RefreshCw size={14} className="shrink-0" />}刷新令牌</button><button onClick={onSync} disabled={pending} className="action-secondary justify-center px-1.5 py-2 text-xs"><RotateCw size={14} className="shrink-0" />同步模型</button>{supportsExport && <button onClick={onExport} disabled={pending} className="action-secondary justify-center px-1.5 py-2 text-xs"><Download size={14} className="shrink-0" />导出 JSON</button>}</div>
