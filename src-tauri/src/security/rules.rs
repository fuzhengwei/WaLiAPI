...
pub async fn seed_builtin_rules(pool: &SqlitePool) -> Result<(), sqlx::Error> {
    let seeds = [
        (
            ...
            "high",
            "疑似密钥/Token",
            "检测 sk-*、ghp_*、AKIA*、AIza*、JWT、Bearer Token 等凭证格式",
            None::<&str>,
        ),
        (
            "b002",
        ...
        (
            ...
            "high",
            "敏感凭证字段",
            "检测 Authorization、Cookie、Session、Secret 等字段名",
            None,
        ),
        (
            "b004",
            ...
            "high",
            "数据库连接串",
            "检测 mysql://、postgres://、mongodb://、redis:// 等连接串",
            None,
        ),
        (
            "b005",
            ...
            "high",
            "云厂商密钥",
            "检测 AWS AKIA、腾讯云 SecretId、阿里云 AccessKey 等",
            None,
        ),
        (
            "b006",
            ...
            "high",
            "敏感文件路径",
            "检测 .env、~/.ssh、id_rsa、.aws/credentials 等敏感路径",
            None,
        ),
        (
            "b007",
            ...
            "critical",
            "SSH 密钥文件",
            "检测 id_rsa、id_ed25519、id_ecdsa 等 SSH 密钥文件名",
            None,
        ),
        (
            "b008",
            ...
            "high",
            "云凭证文件",
            "检测 .aws/credentials、.npmrc、.pypirc、.git-credentials、.netrc 等",
            None,
        ),
        (
            "b009",
        ...
        (
            ...
            "medium",
            "零宽 Unicode 字符",
            "检测 U+200B/200C/200D/2060/FEFF 等不可见字符",
            Some("security.scan_unicode"),
        ),
        (
            "b013",
            ...
            "high",
            "方向控制 Unicode 字符",
            "检测 U+202A-202E、U+2066-2069 等 Bidi 控制字符",
            Some("security.scan_unicode"),
        ),
        (
            "b014",
            ...
            "medium",
            "变体选择符",
            "检测 U+FE00-FE0F、U+E0100-E01EF 等 variation selector",
            Some("security.scan_unicode"),
        ),
        (
            "b015",
        ...
        (
            ...
            "high",
            "公网 IP 探测服务",
            "检测 ifconfig.me、ipinfo.io、ipify.org 等 IP 查询域名",
            Some("security.scan_network"),
        ),
        (
            "b017",
            ...
            "high",
            "可疑外联域名",
            "检测 webhook.site、ngrok、pastebin 等可疑数据接收域名",
            Some("security.scan_network"),
        ),
        (
            "b018",
        ...
        (
            ...
            "high",
            "追踪像素",
            "检测 1x1 图片、track/pixel/beacon 等追踪特征",
            Some("security.scan_network"),
        ),
        (
            "b020",
            ...
            "medium",
            "高风险命令片段",
            "检测 curl/wget/nc/scp/bash -c/python -c 等命令",
            Some("security.scan_tools"),
        ),
        (
            "b021",
        ...
        (
            ...
            "critical",
            "远程脚本执行",
            "检测 curl/wget 下载脚本管道到 bash/sh 的组合",
            Some("security.scan_tools"),
        ),
        (
            "b023",
            ...
            "low",
            "Git 信息泄露",
            "检测 git remote、git config、gh auth token 等可能泄露仓库信息",
            Some("security.scan_tools"),
        ),
        (
            "b024",
...
impl BuiltinRuleRepository {
    ...
    pub async fn reset_to_defaults(pool: &SqlitePool) -> Result<(), sqlx::Error> {
        sqlx::query("DELETE FROM security_builtin_rules")
            .execute(pool)
            .await?;
    ...
    for rule in custom_rules {
        ...
        if matched && rule.rule_type == "blacklist" {
            ...
            let desc = rule
                .description
                .clone()
                .unwrap_or_else(|| format!("匹配自定义{}规则: {}", rule.rule_type, rule.pattern));
            super::scanner::add_finding(
                findings,
                phase,
                "custom",
